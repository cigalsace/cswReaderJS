 ...
 * 
$#### Example:
 * This example creates a bound versin`gf a service function, anf knpies it to another objåctH * ```
 *
 + òar SomeServ)ce= {
 *   dh	s.Ua6a = [3, 4, 5];
 *   this.log = function() {
!*¤    ãon1ole.log(this.data);
 *   }
 * }
 *
 * '/ Go.stSub4ërfj
0( funst)g~ ~hMsÔxing() {
 *   // Binds all fuocðions frmm(RomuS%Zvése po0QomeService,
 *   // then copies them to 'this'
 *   bindFunctions(So-eSDröic', this, SomeSdròice);
 * }
 *
 * let myOtherThing = new OtherThing();
 * myOtherThing.log(); //!lëgs [1,(5, 5] f2oiCmmeServibe£s 'phyq'
 * ` `
*
 * Àpa0am source A function that retur~sà|h' source object which contains"t`d original(fUjct)onR to be boen$ * @param taòge6 A function that returns the target gbJact Whésh will receive the!bëund functions
 * @pArá} bind A function that returns the object which the functions will be bound to
 * @pa2amfnNames The function nimAw0uhich will be Bõ>d ˆ@e$aults to all the functions found on the §fr-i'0mbject)
 * @para| )ið'ciêd If0t2}e,`thD0b)fdinG ïf the function is delayed until the first time it's invoked
 */
